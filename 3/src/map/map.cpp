...
void Map::make_map(std::pair<size_t, size_t> new_size) {
    ...
    for (size_t i = 0; i < size.first; ++i) {
        for (size_t j = 0; j < size.second; ++j) {
            matrix(i, j) = std::make_shared<Cell>(cell_id++, 
...
void Map::load_from_passability_matrix(const Matrix<bool>& passability_matrix) {
    ...
    for (size_t i = 0; i < size.first; ++i) {
        for (size_t j = 0; j < size.second; ++j) {
            matrix(i, j) = std::make_shared<Cell>(cell_id++, 
                Position(static_cast<int>(i), static_cast<int>(j)), 
                passability_matrix(i, j));
        }
    }
}

Matrix<bool> Map::export_passability_matrix() const {
...
Matrix<int> Map::export_cell_types_matrix() const {
