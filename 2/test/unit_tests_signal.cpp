...
TEST(signal_constructor, constructor_and_getters) {
    Signal signal(1, 10);
...
TEST(signal_set_get, set_valid_duration) {
    Signal signal(1, 10);
...
TEST(signal_set_get, set_invalid_level) {
    Signal signal(1, 10);
    EXPECT_THROW(signal.set_level(2), std::invalid_argument);
}

// Initializes Signal object with valid binary string "0" or "1"
TEST(signal_constructor, should_initialize_signal_object_when_given_valid_binary_string) {
...
TEST(signal_constructor, should_initialize_signal_object_when_given_valid_binary_string_2) {
...
TEST(signal_constructor, should_initialize_signal_object_when_given_invalid_string) {
    std::string invalid_binary_string = "qq111aaa";
    EXPECT_THROW({ Signal signal(invalid_binary_string); }, std::invalid_argument);
}

// Handles empty string input
TEST(signal_constructor, should_throw_invalid_argument_when_given_empty_string) {
    std::string empty_string = "";
    EXPECT_THROW({ Signal signal(empty_string); }, std::invalid_argument);
}

// Inverting the signal level from 0 to 1
TEST(signal_inversion, should_set_level_to_1_when_initial_level_is_0) {
    Signal signal;
    signal.inversion();
    EXPECT_EQ(signal.get_level(), 1);
}
...
TEST(signal_inversion, should_invert_the_signal_when_operator_is_used) {
    Signal signal(1, 1);
    ~signal;
    EXPECT_EQ(signal.get_level(), 0);
}
...
TEST(signal_inversion, should_invert_the_signal_when_operator_is_used_double) {
    Signal signal(1, 1);
    ~~signal;
    EXPECT_EQ(signal.get_level(), 1);
}
...
TEST(signal_increase, should_increase_duration_when_value_is_positive) {
    Signal signal;
...
TEST(signal_increase, should_throw_invalid_argument_when_value_is_negative) {
    Signal signal;
...
TEST(signal_increase, increase_overflow) {
...
TEST(signal_decrease, should_decrease_duration_when_value_is_a_positive_integer_less_than_duration) {
    Signal signal(0, 10);
    signal.decrease(5);
...
TEST(signal_format_print, should_correctly_format_and_print_string_when_duration_and_level_are_set_0) {
    Signal signal("000");
    std::wostringstream out;
    signal.format_print(out);
...
TEST(signal_format_print, should_handle_zero_duration_correctly) {
    Signal signal(0, 0);
    std::wostringstream out;
    signal.format_print(out);
    EXPECT_EQ(out.str(), std::wstring(L""));
